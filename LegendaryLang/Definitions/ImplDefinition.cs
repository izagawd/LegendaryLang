...
public class ImplDefinition : IItem, IAnalyzable, IPathResolvable
{
    ...
    /// <summary>
    /// True for compiler-generated impls (MetaSized/Sized) that don't need analysis.
    /// </summary>
        ...
        foreach (var traitAT in traitDef.AssociatedTypes)
        {
            ...
            else
            {
                ...
                if (!boundsMatch)
                {
                    ...
                    analyzer.AddException(new SemanticException(
                        $"Associated type '{traitAT.Name}' bound mismatch: trait declares ':! Sized +{traitBoundsStr}' " +
                        $"but impl declares ':! Sized +{implBoundsStr}'\n{Token.GetLocationStringRepresentation()}"));
                }
    ...
    public static ImplDefinition Parse(Parser parser, NormalLangPath module)
    {
        ...
        while (parser.Peek() is not RightCurlyBraceToken)
        {
            ...
            {
                // Parse: let Output :! Sized = i32; or let Output :! Sized +Copy = i32;
