...
public class EnumTypeDefinition : ComposableTypeDefinition
{
    ...
    public override IRefItem CreateRefDefinition(CodeGenContext context, ImmutableArray<LangPath> genericArguments)
    {
        ...
        var enumStruct = context.LLVMContext.CreateNamedStruct(monomorphizedPath.ToString());
    ...
    public static EnumTypeDefinition Parse(Parser parser, NormalLangPath module)
    {
        ...
        if (parser.Peek() is OperatorToken { OperatorType: Operator.LessThan })
        {
            ...
            while (nextToken is not OperatorToken { OperatorType: Operator.GreaterThan })
            {
                ...
                if (nextToken is CommaToken) { parser.Pop(); nextToken = parser.Peek(); }
                else break;
            }
        ...
        return new EnumTypeDefinition(nameToken.Identity, module, enumToken, variants, genericParameters);
    }
}
